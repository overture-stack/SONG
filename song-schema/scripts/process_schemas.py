...
class ProtobufGenerator(object):
    ...
    def _createSchemaFiles(self, destPath, schemasPath):
        ...
        overturePath = os.path.join(destPath, 'overture')
        if not os.path.exists(overturePath):
            os.mkdir(overturePath)
        ...
        # ga4ghSchemasGooglePath = os.path.join(overtureSchemasPath, 'google')
    ...
    def _doLineReplacements(self, line):
        ...
        """
        # ga4gh packages
        packageString = 'package org.icgc.dcc.song.schema;'
        if packageString in line:
            return line.replace(
                packageString,
                'package overture.schemas.song;')
        importString = 'import "overture/'
        if importString in line:
            return line.replace(
                importString,
                'import "overture/schemas/song/')
        # google packages
    ...
    def _getProtoc(self, destination_path):
        final_dest_path = "{}/protobuf/src/protoc".format(destination_path)
        final_proto_path = self._find_in_path("protoc")
        protocs = [ os.path.realpath(self._find_in_path("protoc")) ]
        protoc = None
        for c in protocs:
            ...
            try:
                (lib, version) = output.decode().split(" ")
                if lib != "libprotoc":
                ...
                print(
                    ...
                    "<version> >= 3.0.0".format(path=c, version=output))
    ...
    def _writeVersionFile(self):
        versionFilePath = "python/overture/schemas/_protocol_version.py"
