...
public class JWTTokenConverter extends JwtAccessTokenConverter {
  ...
  private static Map<String,?> parseContextMap(Map<String, ?> map){
    ...
    throw buildServerException(JWTTokenConverter.class, UNAUTHORIZED_TOKEN, "[@%s] Token is not authorized", timestamp);
  }
  ...
  private static Object extractPrincipleDetails(Map<String, ?> map){
    return parseContextUser(map)
        ...
        .orElseThrow(() -> {
          val timestamp = System.currentTimeMillis();
          log.error("[@{}] JWT Token must have at least one field of: {}, {}",
              CONTEXT_USER_FIELD_NAME, CONTEXT_APPLICATION_FIELD_NAME ,timestamp);
          throw buildServerException(JWTTokenConverter.class, UNAUTHORIZED_TOKEN, "[@%s] Token is not authorized", timestamp);
        });
  }
  ...
  private static <P> Optional<Object> parseContextPrinciple(Map<String, ?> map, Class<P> principleClass, String principleKey){
    val context = parseContextMap(map);
