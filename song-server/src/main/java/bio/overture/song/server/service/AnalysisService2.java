...
public class AnalysisService2 {
  ...
  @Autowired private final AnalysisInfoService analysisInfoService;
  @Autowired private final FileInfoService fileInfoService;
  @Autowired private final SequencingReadInfoService sequencingReadInfoService;
  @Autowired private final VariantCallInfoService variantCallInfoService;
  ...
  @Autowired private final SequencingReadRepository sequencingReadRepository;
  @Autowired private final VariantCallRepository variantCallRepository;
  @Autowired private final SampleSetRepository sampleSetRepository;
  @Autowired private final FileRepository fileRepository;
  @Autowired private final FullViewRepository fullViewRepository;

  @Transactional
  public String create(String studyId, Analysis2 a, boolean ignoreAnalysisIdCollisions) {
   ...
   */
  public List<AbstractAnalysis> getAnalysisByView(
      @NonNull String studyId, @NonNull Set<String> analysisStates) {
    studyService.checkStudyExist(studyId);
    val finalStates = resolveSelectedAnalysisStates(analysisStates);
    val results =
        fullViewRepository.findAllByStudyIdAndAnalysisStateIn(
            studyId, ImmutableList.copyOf(finalStates));
    val analysisTypeMap =
        results.stream().collect(groupingBy(x -> resolveAnalysisType(x.getAnalysisType())));
      ...
      for (val analysesForPartition : partitions) {
        if (analysisType == SEQUENCING_READ) {
          ...
          throw buildServerException(
              getClass(),
              NOT_IMPLEMENTED_YET,
              "The analysisType '%s' has not been implemented yet",
              analysisType);
        }
  ...
  public List<AbstractAnalysis> getAnalysis(
    ...
    // Once hibernate is properly integrated with song, we will be able to use the criteria api to
    // create queries.
    val analysisList =
        finalStates.stream()
  ...
  public List<AbstractAnalysis> idSearch(
      @NonNull String studyId, @NonNull IdSearchRequest request) {
    val analysisList =
  ...
  SequencingRead readSequencingRead(String id) {
    ...
    checkServer(
        result.isPresent(),
        this.getClass(),
        SEQUENCING_READ_NOT_FOUND,
        "The SequencingRead with analysisId '%s' was not found",
        id);
  ...
  VariantCall readVariantCall(String id) {
    ...
    checkServer(
        ...
        "The VariantCall with analysisId '%s' was not found",
        id);
    val experiment = result.get();
  ...
  /**
   * Adds all child entities for each analysis This method should be watched in case performance
   * becomes a problem.
   *
  ...
  private List<AbstractAnalysis> processAnalysisList(List<AbstractAnalysis> analysisList) {
    analysisList.stream().filter(Objects::nonNull).forEach(this::processAnalysis);
    return analysisList;
  }

  /**
   * Adds child entities to analysis This method should be watched in case performance becomes a
   * problem.
   *
  ...
  private void processSequencingReadsInPlace(List<AbstractAnalysis> analyses) {
    val analysisIds =
        analyses.stream().map(AbstractAnalysis::getAnalysisId).collect(toImmutableSet());
    val srMap =
        sequencingReadRepository.findAllByAnalysisIdIn(newArrayList(analysisIds)).stream()
  ...
  private void processVariantCallsInPlace(List<AbstractAnalysis> analyses) {
    ...
    val vcMap =
        variantCallRepository.findAllByAnalysisIdIn(newArrayList(analysisIds)).stream()
