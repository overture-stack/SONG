...
public class ScopeValidator {
  ...
  private boolean isGrantedForStudy(@NonNull String tokenScope, @NonNull String studyId) {
    log.info(
        "Checking study scope '{}', server's scopePrefix='{}', studyId '{}', scopeSuffix='{}'",
        tokenScope,
        getScopePrefix(),
        studyId,
        getScopeSuffix());
    return getSystemScope().equals(tokenScope)
        || getStudyScope(studyId).equals(tokenScope); // short-circuit
  }

  private boolean isGrantedForSystem(@NonNull String tokenScope) {
    log.info(
        "Checking system scope '{}', server's scopePrefix='{}', scopeSuffix='{}'",
        tokenScope,
        getScopePrefix(),
        getScopeSuffix());
    return getSystemScope().equals(tokenScope);
