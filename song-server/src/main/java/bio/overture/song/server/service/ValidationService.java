    ...
    public Optional<String> validate(@NonNull JsonNode payload, @NonNull String studyId) {
        ...
        try {
            ...
            if (!externalValidations.isEmpty()) {
                externalValidations(analysisType.getOptions().getExternalValidation(), payload, studyId);
            }
        ...
        for (ExternalValidation externalValidation : externalValidations) {
            ...
            if (value.isPresent()) {
                ...
                try {
                    ...
                    log.info(String.format("Error occurred while executing external validation against url '%s'.", formattedExternalUrl));
                    throw new ValidationException(errorMessage);
                }
            }
        }
