...
public class AnalysisSchemaSpecification {
  ...
  /**
   * Dynamically generate a query depending on supplied method parameters. If there are names or versions defined, then the query will filter on a combination of those. If none are defined, then a findAll query results.
   */
  public static Specification<AnalysisSchema> buildListQuery(@Nullable Collection<String> names,
    ...
    return (root, query, builder) -> {
      ...
      if (definedNames && definedVersions) {
        return builder.and(
            whereInIdsPredicate(root, NAME, names),
            whereInIdsPredicate(root, VERSION, versions));
  ...
  private static Predicate whereInIdsPredicate(Root<AnalysisSchema> root, String columnName, Collection values) {
