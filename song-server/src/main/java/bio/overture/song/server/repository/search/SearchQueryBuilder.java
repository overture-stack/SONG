...
public class SearchQueryBuilder {
  ...
  public static SearchQueryBuilder createSearchQueryBuilder(
      String studyId, boolean includeInfoField) {
    return new SearchQueryBuilder(studyId, includeInfoField);
  }

  private static String convertToWhereCondition(
      String tableName, String columnName, SearchTerm searchTerm) {
    val sb = new StringBuilder();
    sb.append(tableName + "." + columnName);
    searchTerm
        .getNonLeafKeys()
        .forEach(key -> sb.append(JSON_OBJECT_ARROW).append(surroundSingleQuotes(key)));
  ...
  private static String generateSelectBaseQuery(boolean includeInfoField, String studyId) {
    ...
    sb.append(
        String.format(
            "SELECT CAST(analysis.id AS VARCHAR) AS %s ",
            InfoSearchResponseColumns.ANALYSIS_ID.toString()));
    if (includeInfoField) {
      sb.append(
          String.format(
              ", CAST(info.info AS VARCHAR) AS %s ", InfoSearchResponseColumns.INFO.toString()));
    }
    sb.append(
        format(
            "FROM analysis INNER JOIN %s ON analysis.id = info.id WHERE info.id_type = 'Analysis' AND analysis.study_id = '%s'",
            TABLE_NAME, studyId));
    return sb.toString();
  }
}
