...
public class AnalysisTypePageableResolver2 implements HandlerMethodArgumentResolver {
  ...
  private static AnalysisTypePageable2 buildAnalysisTypePageable(String limitString, String offsetString,
    ...
    return AnalysisTypePageable2.builder()
        .limit(limit)
        .offset(offset)
        .sort(sort)
        .build();
  ...
  private static void validateNonNegativeParam(@NonNull String paramName, int paramValue){
    checkServer(paramValue>=0, AnalysisTypePageableResolver2.class,
        MALFORMED_PARAMETER,
  ...
  private static Direction resolveSortOrder(String sortOrder, Direction defaultSortOrder){
    ...
    if (!isEmpty(sortOrder)){
      val result = stream(Direction.values())
          ...
          .findFirst();
      direction = checkServerOptional(result, AnalysisTypePageableResolver2.class, MALFORMED_PARAMETER,
