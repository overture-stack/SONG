...
public class SearchTerm {
  ...
  public void setValue(String value) {
    checkServer(
        ...
        this.getClass(),
        SEARCH_TERM_SYNTAX,
        "value for key '%s' must be non-empty and non-null",
        getKey());
    this.value = value;
  }

  /**
   * Get the Non leaf keys of the keyChain. I.e if the keyChain is "country,province,city", then the
   * nonleaf keys are country,province
   *
  ...
  public List<String> getNonLeafKeys() {
    if (keyChain.size() < 1) {
      ...
      return keyChain.stream().limit(keyChain.size() - 1).collect(toImmutableList());
    }
  }

  /**
   * Get the Leaf keys of the keyChain. I.e if the keyChain is "country,province,city", then the
   * leaf key is "city"
   *
   * @return string
   */
  ...
  private static List<String> parseKeyChain(String key) {
    checkServer(
        ...
        SearchTerm.class,
        SEARCH_TERM_SYNTAX,
        "The key '%s' is not acceptable. There must be at least one non-whitespace character",
        key);
  ...
  public static List<SearchTerm> parseSearchTerms(String... keyValuePairs) {
    return stream(keyValuePairs).map(SearchTerm::parseSearchTerm).collect(toImmutableList());
  }

  public static List<SearchTerm> createMultiSearchTerms(
      @NonNull String key, @NonNull List<String> values) {
    return values.stream().map(v -> createSearchTerm(key, v)).collect(toImmutableList());
  }
}
