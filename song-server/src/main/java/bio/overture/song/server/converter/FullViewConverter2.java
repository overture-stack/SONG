...
public class FullViewConverter2 {
   ...
   */
  public static List<Analysis2> processAnalysisForType(
      @NonNull List<FullView> fullViewEntities,
      @NonNull AnalysisTypes analysisType,
      @NonNull Function<String, Analysis2> analysisInstantiatorCallback) {
    ...
    val outputList = ImmutableList.<Analysis2>builder();
  ...
  private static List<FileEntity> extractFiles(List<FullView> fullViews) {
    return ImmutableList.copyOf(transformImmutableSet(fullViews, FullViewConverter2::extractFile));
  }
  ...
  private static Optional<Specimen> extractSpecimenForSample(
    ...
    return fullViews.stream()
        ...
        .map(FullViewConverter2::extractSpecimen)
        .findFirst();
  }
  ...
  private static Optional<Donor> extractDonorForSpecimen(
    ...
    return fullViews.stream()
        ...
        .map(FullViewConverter2::extractDonor)
        .findFirst();
  }
  ...
  private static List<Sample> extractSamples(List<FullView> fullViews) {
    return ImmutableList.copyOf(
        transformImmutableSet(fullViews, FullViewConverter2::extractSample));
  }
}
