...
public class InfoSearchTest {
  ...
  private static final Set<String> TEST_PAYLOAD_FILENAMES_1 =
      newHashSet(
          "testData_0.json",
          "testData_1.json",
          "testData_2.json",
  ...
  private RandomGenerator randomGenerator =
      createRandomGenerator(InfoSearchTest.class.getSimpleName());
  ...
  public void testInfoSearchResponse() {
    val info = JsonNodeBuilders.object().with("key1", "value1").with("key2", "value2").end();
  ...
  @SneakyThrows
  private void runBasicTermSearchTest(
      Supplier<List<InfoSearchResponse>> responseSupplier, boolean shouldHaveInfo) {
    ...
    if (shouldHaveInfo) {
      assertEquals(
          infoSearchResponse1, this.analysisRespMap.get(infoSearchResponse1.getAnalysisId()));
    }
    assertEquals(infoSearchResponse1.hasInfo(), shouldHaveInfo);
  }

  private InfoSearchResponse loadAndCreateResponse1(String study, String payloadString)
      throws Exception {
  ...
  @SneakyThrows
  private List<InfoSearchResponse> search(
      String studyId, boolean includeInfo, String... searchTermStrings) {
  ...
  @SneakyThrows
  private List<InfoSearchResponse> search2(
      String studyId, boolean includeInfo, String... searchTermStrings) {
    val searchTerms = parseSearchTerms(searchTermStrings);
