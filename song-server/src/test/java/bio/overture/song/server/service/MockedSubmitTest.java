...
public class MockedSubmitTest {
  ...
  public void submit_MissingAnalysisTypeName_MalformedParameter(){
    // Setup
    doNothing().when(studyService).checkStudyExist(anyString());
    when(validationService.validateAnalysisTypeVersion(isA(AnalysisTypeId.class))).thenCallRealMethod();

    val payload = JsonUtils.toJson(Payload.builder()
        .study("anyStudy")
        .analysisType(AnalysisTypeId.builder()
            .build())
        .build());
  ...
  public void submit_StudyDne_StudyIdDoesNotExist(){
    ...
    doThrow( buildServerException(getClass(), STUDY_ID_DOES_NOT_EXIST, "study dne"))
        .when(studyService).checkStudyExist(anyString());
  ...
  public void submit_invalidPayload_SchemaViolation(){
    ...
    val invalidPayload = toJson(Payload.builder()
        .study(studyId)
        .analysisType(AnalysisTypeId.builder()
            .name(VARIANT_CALL.getAnalysisTypeName())
            .build())
        .build()
    );
    ...
    // Verify
    assertSongError(() ->
        uploadService.submit(studyId, invalidPayload, false), SCHEMA_VIOLATION);
