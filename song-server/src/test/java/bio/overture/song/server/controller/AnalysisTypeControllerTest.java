...
public class AnalysisTypeControllerTest {
  ...
  @Autowired private Supplier<Schema> analysisTypeRegistrationSchemaSupplier;
   ...
   * @param actual
   */
  private void assertRegisteredSchemaCorrectDetails(AnalysisType expected, AnalysisType actual) {
  ...
  public void register_multipleNonExistingName_success() {
    ...
    assertRegisteredSchemaCorrectDetails(expectedAnalysisTypeUpdate2, actualAnalysisTypeUpdate2);
  ...
  @Test
  @SneakyThrows
  public void getRegistrationSchema_existing_success() {
  ...
  public void register_nameRegistration_illegalAnalysisTypeName() {
    ...
    val registerRequest =
        RegisterAnalysisTypeRequest.builder().name(name).schema(inputValidSchema).build();
    val songErrorResponse =
        endpointTester
            .registerAnalysisTypePostRequestAnd(registerRequest)
            .assertIsError()
            .assertServerError(ILLEGAL_ANALYSIS_TYPE_NAME)
            .getResponse();
    val songError = parseErrorResponse(songErrorResponse);
    val actualMessage = songError.getMessage();
    assertEquals(
    ...
    return range(0, repeats * repeats)
        ...
        .map(
            i -> {
              ...
              val out = analysisTypeService.register(name,new Options(), schema);
              return out;
            })
        .collect(toImmutableList());
  }
